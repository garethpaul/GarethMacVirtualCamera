...
def main():
    ...
    require("CI-equivalent unsigned compile" in readme_text and "./scripts/build_unsigned.sh" in readme_text and "./scripts/scan_build_log.py build-Debug.log build-Release.log" in readme_text and ".build/Xcode" in readme_text and "BUILD_OUTPUT_PATH" in readme_text,
            ...
            failures)
    require("verifies the built app products contain the embedded system extension, declared executables, CoreMediaIO extension metadata, and bundled video" in readme_text,
    ...
    require("not running from `/Applications/GarethVideoCam.app`" in readme_text and "refreshes readiness when the app becomes active" in readme_text and "shows and copies a readiness summary and checklist" in readme_text and "primary System Settings approval shortcut" in readme_text and "reveal the app and embedded extension in Finder" in readme_text and "diagnostics snapshot" in readme_text and "generation timestamp" in readme_text and "macOS version" in readme_text and "bundle identifiers" in readme_text and "expected and current app paths" in readme_text and "app and extension quarantine status" in readme_text and "host app bundle identifier does not match the expected identifier" in readme_text and "missing the System Extension entitlement" in readme_text and "embedded extension executable or CMIO Mach service metadata is missing" in readme_text and "embedded `video.mp4` resource is missing or empty" in readme_text and "bundled system extension signature is invalid" in readme_text and "Team IDs" in readme_text and "bundled extension executable and CMIO Mach service metadata" in readme_text and "pending request direction" in readme_text and "last recorded failure" in readme_text and "timestamped recent request activity with severity" in readme_text,
            ...
            failures)
    require("collect_runtime_diagnostics.sh" in readme_text and "Info.plist bundle versions and identifiers" in readme_text and "bundled-video byte size, checksum, metadata" in readme_text and "expected application-location and bundle identifier checks" in readme_text and "quarantine attributes" in readme_text and "matching Team IDs" in readme_text and "Gatekeeper assessment" in readme_text and "signed entitlements" in readme_text and "explicit host System Extension entitlement checks" in readme_text and "counted runtime-readiness summary" in readme_text and "embedded system-extension executable and CMIO Mach service metadata" in readme_text and "systemextensionsctl" in readme_text and "registration presence and full list output" in readme_text and "camera device inventory" in readme_text and "running app/extension processes" in readme_text and "unified-log window" in readme_text and "system-extension/CMIO log context" in readme_text,
    ...
    require("GarethVideoCam.app" in verify_build_products_source and "com.garethpaul.GarethVideoCam.Extension.systemextension" in verify_build_products_source and "Contents/Library/SystemExtensions" in verify_build_products_source and "Contents/Resources/video.mp4" in verify_build_products_source and "read_bundle_identifier" in verify_build_products_source and "read_bundle_executable" in verify_build_products_source and "verify_bundle_executable" in verify_build_products_source and "read_extension_mach_service_name" in verify_build_products_source and "verify_extension_cmio_metadata" in verify_build_products_source and "CMIOExtensionMachServiceName" in verify_build_products_source and "Contents/Info.plist" in verify_build_products_source and "Contents/MacOS" in verify_build_products_source and "CFBundleExecutable" in verify_build_products_source and "plistlib" in verify_build_products_source and "PlistBuddy" not in verify_build_products_source and "Debug Release" in verify_build_products_source,
            "build-product verifier should check app, embedded extension, bundle identifiers, declared executables, CoreMediaIO metadata, and bundled video",
            failures)
    require(verify_build_products_path.stat().st_mode & 0o111,
            "build-product verifier script should be executable",
            failures)
    require("write_product_fixture" in verify_build_products_test_source and "write_executable_fixture" in verify_build_products_test_source and "com.example.WrongExtension" in verify_build_products_test_source and "Unexpected Debug extension bundle identifier" in verify_build_products_test_source and "Missing or empty Debug bundled video resource" in verify_build_products_test_source and "Missing or non-executable Debug extension executable" in verify_build_products_test_source and "Missing Debug extension CMIOExtensionMachServiceName" in verify_build_products_test_source and "Build-product verifier tests passed." in verify_build_products_test_source,
            "build-product verifier should have fixture coverage for passing products, bundle identifier failures, missing executables, missing CoreMediaIO metadata, and missing bundled video",
            failures)
