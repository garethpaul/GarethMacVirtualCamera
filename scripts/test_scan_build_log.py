...
def test_fails_on_appintents_error_notice():
    result = run_scanner(
        "appintentsmetadataprocessor error: Metadata extraction skipped. "
        "No AppIntents.framework dependency found.\n"
    )
    require(result.returncode == 1, result.stdout + result.stderr)
    require("appintentsmetadataprocessor error" in result.stdout, result.stdout)


def test_fails_on_appintents_warning_with_embedded_error():
    result = run_scanner(
        "appintentsmetadataprocessor warning: Metadata extraction skipped. "
        "No AppIntents.framework dependency found. SwiftCompile error: real source error\n"
    )
    require(result.returncode == 1, result.stdout + result.stderr)
...
def test_fails_on_analyze_failed_banner():
    result = run_scanner("** ANALYZE FAILED **\n")
    require(result.returncode == 1, result.stdout + result.stderr)
    require("** ANALYZE FAILED **" in result.stdout, result.stdout)


def test_fails_on_clean_failed_banner():
    result = run_scanner("** CLEAN FAILED **\n")
    require(result.returncode == 1, result.stdout + result.stderr)
