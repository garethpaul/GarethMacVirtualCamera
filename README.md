...
6. Confirm the diagnostics report `Runtime readiness result: ready`, `Extension registration entry present: yes`, `Extension registration activated enabled: yes`, `Application group match ready: yes`, and `Expected virtual camera device present: yes`.
